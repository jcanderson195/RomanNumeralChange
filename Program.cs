...
namespace ConsoleApplication1
{
    public class RomanConversions
    {
        ...
        public void ConvertFromRome(string UserInput)
        {
            ...
            {
                char firstCharacter = this.NumeralList.ElementAt<char>(i);
                char secondCharacter = this.NumeralList.ElementAt<char>(i + 1);
                char thirdCharacter = this.NumeralList.ElementAt<char>(i - 1);
                if (i != 1)
                {
                    if (firstCharacter == thirdCharacter)
                    {
                        this.decConvertTotal = this.decConvertTotal + this.RomeConverter(firstCharacter);
                    }
                    else if ((firstCharacter == thirdCharacter ? false : firstCharacter != secondCharacter))
                    {
                        if (this.RomeConverter(secondCharacter) <= this.RomeConverter(firstCharacter))
                        {
                            this.decConvertTotal = this.decConvertTotal + this.RomeConverter(firstCharacter);
                        }
                        else
                        {
                            this.decConvertTotal = this.decConvertTotal + (this.RomeConverter(secondCharacter) - this.RomeConverter(firstCharacter));
                            i++;
                        }
                    }
                }
